<template>
  <div class="captcha-text">
    <div class="captcha-text__image" v-if="textImage">
      <img :src="textImage" alt="验证码图片" @load="handleImageLoad" />
    </div>
    
    <div v-else class="captcha-text__generate">
...
const props = withDefaults(defineProps<CaptchaTextProps>(), {
  texts: () => [],
  requiredCount: 1,
  caseSensitive: false,
  placeholder: '请输入验证码',
...
const submitInput = async () => {
  ...
  if (input === expected) {
    ...
    const err = new Error('验证码不正确，请重试');
    setError(err);
    emit('error', err);
    userInput.value = '';
    refreshText();

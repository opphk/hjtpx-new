...
interface PuzzleCaptchaComponentProps extends CaptchaComponentProps {
...
const PuzzleCaptcha: React.FC<PuzzleCaptchaComponentProps> = ({
  captchaData,
  onSuccess,
  onFail,
  onClose,
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: async (_, gestureState) => {
        ...
        const finalTrack = [
          ...track,
          { x: puzzlePosition, y: gestureState.dy, timestamp }
        ];

        try {
          const userResponse: PuzzleResponse = {
            offsetX: puzzlePosition,
            offsetY: 0,
            track: finalTrack,
          };

          const response = await fetch(
            `${captchaData.captchaId ? '/api/v1/captcha/puzzle/verify' : ''}`,
            {
              method: 'POST',
              headers: {
  ...
  const showPuzzlePiece = puzzlePosition > 10;

  return (
    <View style={[styles.container, { width, height: height + 80 }]}>
      <View style={styles.header}>
        <Text style={styles.title}>拼图验证码</Text>
        ...
        {!captchaData.puzzleImage && (
          <Animated.View
            style={[
              ...
              isLoading ? styles.puzzleLoading : styles.puzzleDefault,
            ]}
            {...panResponder.panHandlers}
          >
            {isLoading ? (
      ...
      <View style={styles.sliderContainer}>
        ...
        <Text style={styles.hint}>拖动滑块将拼图放入正确位置</Text>
      </View>

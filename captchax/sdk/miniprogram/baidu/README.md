...
CaptchaX 百度小程序 SDK 是一款功能强大的多类型验证码解决方案，支持 6 种不同类型的验证码组件，帮助开发者快速在百度小程序中集成安全可靠的验证功能。
...
<captcha
  ...
  appId="{{appId}}"
  type="slider"
  bind:onVerify="onVerify"
  bind:onClose="onCaptchaClose"
  bind:onError="onCaptchaError"
/>
```
...
1. 在百度开发者工具中导入 `example/` 文件夹
2. 配置您的 AppId（在 app.js 的 globalData 中）
3. 启动项目预览

## 百度小程序 API 差异

与支付宝小程序相比，百度小程序有以下主要差异：

1. **API 调用方式**：
   - 支付宝：`my.request()`, `my.getSystemInfoSync()`
   - 百度：`swan.request()`, `swan.getSystemInfo()`
...
A: 百度小程序需要使用 `bind:` 前缀来绑定组件事件，例如 `bind:onVerify`、`bind:onClose` 等。
